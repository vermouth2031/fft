`timescale 1ns/1ps
module analyzer_core(input wire src_clk,fft_clk,input wire rst,
 ...
 .READ_MODE("fwft"),.FIFO_READ_LATENCY(0),.CDC_SYNC_STAGES(2),.DOUT_RESET_VALUE("0"),.USE_ADV_FEATURES("0000")) input_fifo(
 .rst(rst),.wr_clk(src_clk),.wr_en(valid&&!full&&!wr_busy),.din(iq),.full(full),.wr_rst_busy(wr_busy),
 ...
 result_builder rb(.clk(src_clk),.rst(rst),.tick(tick),.epoch(epoch),.config_id(config_id),.hann(hann),.integrity_flags(integrity_flags),
